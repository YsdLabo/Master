...
void InwheelMotorController::init()
{
	...
	motor_left->motor_on();
	while(motor_right->get_pulse_count(&count_right_i));
	while(motor_left->get_pulse_count(&count_left_i));
	count_right = 0;
	count_left = 0;
	distance_right = 0.0;
	distance_left = 0.0;
...
{
	int count_r, count_l;
	if(motor_right->get_pulse_count(&count_r) && motor_left->get_pulse_count(&count_l))
	{
